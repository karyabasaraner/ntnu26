...
TEST(ShareDictTest, ReaderReadAbsoluteReturnsFramePayloadAndTimestamp) {
    ...
    // THEN: The reader returns the frame payload and timestamp
...
TEST(ShareDictTest, WriterPublishesPayloadToSharedMemory) {
    ...
    // THEN: The reader returns the published payload and timestamp
    EXPECT_EQ(entry.key, "accelerometer");
    EXPECT_EQ(entry.data, payload);
    EXPECT_EQ(entry.head, 0U);
